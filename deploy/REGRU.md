...
CREATE USER crm WITH PASSWORD 'НАДЁЖНЫЙ_ПАРОЛЬ';
...
DATABASE_URL=postgresql://crm:НАДЁЖНЫЙ_ПАРОЛЬ@localhost:5432/crm
...
```bash
cd /var/www/sdr-crm
chown -R www-data:www-data /var/www/sdr-crm
...
```bash
cp deploy/sdr-crm-api.service /etc/systemd/system/
systemctl daemon-reload
systemctl enable sdr-crm-api
systemctl start sdr-crm-api
systemctl status sdr-crm-api
```
...
sudo bash /var/www/sdr-crm/deploy/scripts/install-ops.sh

# Просмотр логов API
journalctl -u sdr-crm-api -f
tail -f /var/log/sdr-crm/api.log

# Ошибки за последний час
journalctl -u sdr-crm-api --since "1 hour ago" -p err
```
...
systemctl list-timers sdr-crm-backup.timer
```

Вручную:

```bash
sudo /var/www/sdr-crm/deploy/scripts/backup-db.sh
sudo /var/www/sdr-crm/deploy/scripts/backup-verify.sh latest
```

Восстановление:

```bash
sudo /var/www/sdr-crm/deploy/scripts/restore-db.sh /var/backups/sdr-crm/crm_YYYYMMDD_HHMMSS.sql.gz
```
...
```bash
sudo bash /var/www/sdr-crm/deploy/scripts/harden-vps.sh
```
...
| Симптом | Решение |
|---------|---------|
| 502 Bad Gateway | `systemctl status sdr-crm-api`, проверить `server/.env` |
