...
function RevokeForm({ t }: { t: Record<string, string> }) {
  ...
  async function submit() {
    setErr(""); setMsg("");
    try {
      const r = await api.publicRevoke({ phone });
      setMsg(r.message);
      setPhone("");
    } catch (e) {
  ...
  return (
    <div className={`mt-10 rounded-xl border p-5 ${t.surface} ${t.border}`}>
      ...
      <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+7 ..."
        className={`mt-3 w-full rounded-lg border px-3 py-2 text-sm ${t.input}`} />
      {err && <p className="text-sm text-rose-500 mt-2">{err}</p>}
      {msg && <p className="text-sm text-teal-600 mt-2">{msg}</p>}
      <button onClick={submit} className="mt-3 px-4 py-2 rounded-lg bg-teal-600 text-white text-sm font-medium hover:bg-teal-700">
        Отозвать согласие
      </button>
    </div>
  );
