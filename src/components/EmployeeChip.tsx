...
export function EmployeeAvatar({
  ...
  const ringCls = ring ? "bio-avatar bio-avatar--ring" : "bio-avatar";
  if (member.avatar) {
    return (
      <img
        src={member.avatar}
        alt=""
        className={`${cls} rounded-full object-cover shrink-0 ${ringCls}`}
      />
    );
  }
  return (
    <span className={`${cls} rounded-full inline-flex items-center justify-center font-semibold shrink-0 ${ringCls}`}>
...
export function EmployeeChip({
  ...
  return (
    <Tag
      ...
      onClick={onClick}
      className={`bio-employee-chip inline-flex items-center gap-1.5 rounded-full border transition ${
        compact ? "px-1.5 py-0.5" : "px-2 py-1"
      } ${
        selected
          ? "bio-employee-chip--selected"
